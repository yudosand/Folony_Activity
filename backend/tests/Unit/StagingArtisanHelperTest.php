...
class StagingArtisanHelperTest extends TestCase
{
    private string $root;
    ...
    {
        parent::setUp();
        $this->source = dirname(__DIR__, 3) . '/scripts/templates/staging-artisan-once.php.template';
...
PHP);
    ...
    private function runHelper(string $method, ?string $token = null, string $host = 'staging-absent.folony.co.id', string $fail = ''): string
    {
        ...
        $process = new Process([PHP_BINARY, '-r', $code], $this->root, ['FAIL_COMMAND' => $fail]);
        $process->mustRun();
    ...
    {
        $this->assertStringContainsString('Jalankan Artisan Staging', $this->runHelper('GET'));
        $this->assertStringContainsString('STATUS=404', $this->runHelper('POST', 'wrong'));
        $this->assertStringContainsString('STATUS=404', $this->runHelper('POST', null, 'absent.folony.co.id'));
